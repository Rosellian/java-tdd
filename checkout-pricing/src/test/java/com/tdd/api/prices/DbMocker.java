...
public class DbMocker {
    ...
    public void mockDefaultPriceList() {
        PriceList pl = new PriceList(DEFAULT_NAME, V_1, List.of());
        mockLoadList(List.of(pl));
    }

    public void mockLoadList(List<PriceList> priceLists) {
        when(jdbc.query(LOAD_PRICE_LIST, priceListRowMapper, DEFAULT_NAME))
    ...
    public void mockLoadPrices() {
        ...
        when(jdbc.query(LOAD_PRICES, priceRowMapper, DEFAULT_NAME))
                .thenReturn(prices);
    }
    ...
    public void mockSaveListException() {
        when(jdbc.update(SAVE_PRICE_LIST, DEFAULT_NAME, V_1))
                .thenThrow(new DataAccessException("DB error") {});
    }

    public void mockDeletePricesException() {
        when(jdbc.update(DELETE_PRICES_FOR_LIST, DEFAULT_NAME))
                .thenThrow(new DataAccessException("DB error") {});
    }

    public void mockSavePriceException() {
        when(jdbc.update(SAVE_PRICE, DEFAULT_NAME, "A", 50.0))
                .thenThrow(new DataAccessException("DB error") {});
    }

    public void mockList() {
        when(jdbc.queryForList(LIST_PRICE_LISTS, String.class))
                .thenReturn(List.of(DEFAULT_NAME, PRICE_LIST_A_NAME));
    }
